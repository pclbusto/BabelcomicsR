...
pub fn build(comic_ids: Vec<i64>, pool: SqlitePool) -> gtk::Widget {
    ...
    // ════════════════════════ LEFT COLUMN ════════════════════════
    let left_box = gtk::Box::builder().orientation(gtk::Orientation::Vertical).build();
    let left_cover = gtk::Box::builder()
        .height_request(COVER_HEIGHT).halign(gtk::Align::Fill).hexpand(true)
        .overflow(gtk::Overflow::Hidden).css_classes(["comic-cover-container"])
        .build();
    append_placeholder(&left_cover, 64);

    let left_filename = gtk::Label::builder()
        .label("Selecciona un cómic").ellipsize(gtk::pango::EllipsizeMode::Middle)
        .halign(gtk::Align::Center).margin_top(6).margin_bottom(4).css_classes(["caption", "dim-label"])
        .build();

    let left_list = gtk::ListBox::builder().selection_mode(gtk::SelectionMode::Single).css_classes(["navigation-sidebar"]).build();
    let left_scroll = gtk::ScrolledWindow::builder().vexpand(true).child(&left_list).build();
    ...
    // ════════════════════════ RIGHT COLUMN ════════════════════════
    let right_box = gtk::Box::builder().orientation(gtk::Orientation::Vertical).build();
    let right_cover = gtk::Box::builder()
        .height_request(COVER_HEIGHT).halign(gtk::Align::Fill).hexpand(true)
        .overflow(gtk::Overflow::Hidden).css_classes(["comic-cover-container"])
        .build();
    let right_info = gtk::Label::builder()
        .label("Esperando selección…").halign(gtk::Align::Center).margin_top(6).css_classes(["caption"])
        .build();
    let candidates_box = gtk::Box::builder().orientation(gtk::Orientation::Horizontal).spacing(8).margin_top(8).build();
    let candidates_scroll = gtk::ScrolledWindow::builder()
        ...
        .hexpand(true).min_content_height(THUMB_SMALL_H + 20).child(&candidates_box)
        .build();
    ...
    // ── Row selection logic ──────────────────────────────────────────────────
    {
        ...
        let comic_id = {
            ...
            let filename = std::path::Path::new(&comic.path).file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_else(|| comic.path.clone());
        ...
        if let Some(c_matches) = cached {
            *matches.borrow_mut() = c_matches.clone();
            populate_right_column(&c_matches, 0, &right_cover, &right_info, &candidates_box, &apply_btn, matches.clone(), selected_match_idx.clone(), p_cache.clone());
            return;
        }

        clear_box(&right_cover);
        right_cover.append(&adw::Spinner::builder().halign(gtk::Align::Center).valign(gtk::Align::Center).build());
        ...
        run_in_background(tokio::runtime::Handle::current(), async move {
            ...
            if s_gen.get() != generation { return; }
            match result {
                Ok(new_matches) => {
                    cache_cb.lock().unwrap().insert(comic_id, new_matches.clone());
                    *m_ui.borrow_mut() = new_matches.clone();
                    populate_right_column(&new_matches, 0, &r_cov, &r_inf, &c_box, &a_btn, m_ui, smi, pc_cb);
                }
                Err(e) => {
                    clear_box(&r_cov); append_placeholder(&r_cov, 64);
    ...
    // ── Background Prefetcher ────────────────────────────────────────────────
    {
        ...
        tokio::runtime::Handle::current().spawn(async move {
            loop {
                ...
                if cache_bg.lock().unwrap().contains_key(&next_id) { continue; }
                if let Ok(res) = crate::helpers::suggestion_service::suggest_best_matches(&pool_bg, next_id, 10).await {
    ...
    // ── Apply logic ──────────────────────────────────────────────────────────
    {
        ...
        apply_btn.connect_clicked(move |btn| {
            ...
            let info_id = matches.borrow().get(selected_match_idx.get()).map(|m| m.id_comicbook_info);
            ...
            if let (Some(cid), Some(iid)) = (comic_id, info_id) {
                ...
                run_in_background(tokio::runtime::Handle::current(), async move {
                    crate::repositories::ComicbookRepository::new(&p).set_info(cid, Some(iid)).await
                }, move |_| {
                    if idx + 1 < total {
                        if let Some(row) = ll.row_at_index((idx + 1) as i32) { ll.select_row(Some(&row)); }
                    }
                });
            }
        });
    ...
    // ── Initial load ──────────────────────────────────────────────────────────
    {
        ...
        run_in_background(tokio::runtime::Handle::current(), async move {
            ...
            for id in &comic_ids {
                if let Ok(Some(v)) = repo.get_view_by_id(*id).await { views.push(v); }
            }
            views
        }, move |views| {
            { *cv_init.borrow_mut() = views.clone(); }
            for v in &views { ll_init.append(&build_list_row(v)); }
            if let Some(first) = ll_init.row_at_index(0) { ll_init.select_row(Some(&first)); }
        });
    }
...
fn clear_box(b: &gtk::Box) { while let Some(c) = b.first_child() { b.remove(&c); } }

fn append_placeholder(b: &gtk::Box, px: i32) {
    let img = gtk::Image::builder().icon_name("image-x-generic-symbolic").pixel_size(px).opacity(0.3).build();
    b.append(&img);
}
...
fn load_comic_cover_async(comic_id: i64, target: gtk::Box) {
    ...
    run_in_background(tokio::runtime::Handle::current(), async move { tokio::fs::read(&thumb_path).await.ok() }, move |bytes_opt| {
...
fn build_list_row(view: &ComicbookView) -> gtk::ListBoxRow {
    let row_box = gtk::Box::builder().orientation(gtk::Orientation::Horizontal).spacing(10).margin_top(4).margin_bottom(4).margin_start(6).margin_end(6).build();
    let thumb_container = gtk::Box::builder().width_request(48).height_request(THUMB_SMALL_H).overflow(gtk::Overflow::Hidden).css_classes(["comic-cover-container"]).build();
    ...
    let label_text = if let Some(t) = &view.titulo { t.clone() } else { std::path::Path::new(&view.path).file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_else(|| view.path.clone()) };
    row_box.append(&thumb_container);
    row_box.append(&gtk::Label::builder().label(&label_text).ellipsize(gtk::pango::EllipsizeMode::End).halign(gtk::Align::Start).hexpand(true).max_width_chars(24).build());
    gtk::ListBoxRow::builder().child(&row_box).build()
}

#[allow(clippy::too_many_arguments)]
fn populate_right_column(new_matches: &[UnifiedSuggestion], active_idx: usize, right_cover: &gtk::Box, right_info: &gtk::Label, candidates_box: &gtk::Box, apply_btn: &gtk::Button, matches: Rc<RefCell<Vec<UnifiedSuggestion>>>, smi: Rc<Cell<usize>>, pc: Arc<Mutex<std::collections::HashMap<(i64, i32), gdk_pixbuf::Pixbuf>>>) {
    if new_matches.is_empty() {
        clear_box(right_cover); append_placeholder(right_cover, 64);
        right_info.set_label("Sin matches encontrados");
        apply_btn.set_sensitive(false); return;
    }
    ...
    for (i, m) in new_matches.iter().enumerate() {
        ...
        gesture.connect_pressed(move |g, _, _, _| {
            ...
            while let Some(c) = child {
                if j == idx { c.add_css_class("selected"); } else { c.remove_css_class("selected"); }
                child = c.next_sibling(); j += 1;
            }
...
fn show_match_in_cover(matches: &[UnifiedSuggestion], idx: usize, right_cover: &gtk::Box, right_info: &gtk::Label, pc: Arc<Mutex<std::collections::HashMap<(i64, i32), gdk_pixbuf::Pixbuf>>>) {
    ...
    right_info.set_label(&format!("{}{} #{} ({:.0}%)", method, m.titulo, m.numero.as_deref().unwrap_or("?"), m.similarity * 100.0));
    ...
    let r_c = m.ruta_cover.clone(); let u_o = m.url_original.clone();
    let n_v = m.nombre_volume.clone().unwrap_or_default(); let i_v = m.id_volume.unwrap_or(0);
    let pc_clone = pc.clone();
    run_in_background(tokio::runtime::Handle::current(), async move {
        let bytes = crate::helpers::paths::read_comicbook_info_cover_bytes(r_c.as_deref(), u_o.as_deref(), &n_v, i_v).await?;
        tokio::task::spawn_blocking(move || -> Option<(Vec<u8>, i32, i32, i32)> {
            let img = image::load_from_memory(&bytes).ok()?;
            let scaled = img.resize(u32::MAX, target_h as u32, image::imageops::FilterType::Triangle);
            let rgb = scaled.into_rgb8(); let (w, h) = (rgb.width() as i32, rgb.height() as i32);
            Some((rgb.into_raw(), w, h, w * 3))
        }).await.ok()?
    }, move |pixels_opt| {
        if let (Some((data, w, h, rs)), Some(container)) = (pixels_opt, cover_weak.upgrade()) {
            let pixbuf = gdk_pixbuf::Pixbuf::from_bytes(&glib::Bytes::from_owned(data), gdk_pixbuf::Colorspace::Rgb, false, 8, w, h, rs);
            pc_clone.lock().unwrap().insert((info_id, target_h), pixbuf.clone());
            clear_box(&container);
            let pic = gtk::Picture::for_pixbuf(&pixbuf);
            pic.set_content_fit(gtk::ContentFit::Contain);
...
fn build_candidate_thumb(m: &UnifiedSuggestion, is_active: bool, pc: Arc<Mutex<std::collections::HashMap<(i64, i32), gdk_pixbuf::Pixbuf>>>) -> gtk::Box {
    let outer = gtk::Box::builder().orientation(gtk::Orientation::Vertical).spacing(2).build();
    if is_active { outer.add_css_class("selected"); }
    outer.add_css_class("card");
    let cover_box = gtk::Box::builder().width_request(60).height_request(THUMB_SMALL_H).overflow(gtk::Overflow::Hidden).css_classes(["comic-cover-container"]).build();
    ...
    outer.append(&gtk::Label::builder().label(&format!("{:.0}%", m.similarity * 100.0)).css_classes(["caption"]).halign(gtk::Align::Center).build());
    outer
}
