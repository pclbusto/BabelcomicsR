...
pub fn run(pool: SqlitePool) {
    ...
    app.connect_activate(move |app| {
        ...
        crate::helpers::extraction_registry::cleanup_stale();
        ...
        if app.windows().is_empty() {
            ...
            // para que esté listo cuando el usuario lo abra (sin bloquear el arranque).
            {
                ...
                glib::idle_add_local_once(move || {
                    let _ = cache.set(build_preferences_dialog(pool_prefs));
                });
            }
...
fn build_preferences_dialog(pool: SqlitePool) -> adw::PreferencesDialog {
                ...
                async move {
                    ...
                    let computed = tokio::task::spawn_blocking(move || {
                        ...
                        for (id, path) in &entries {
                            let thumb = crate::helpers::paths::comic_thumbnail_path(*id, card_size);
                            let bytes = if thumb.exists() {
                                ...
                                match crate::helpers::extractor::extract_cover(path) {
                                    Ok(b) => b,
                                    Err(_) => continue,
                                }
                            };
...
fn populate_dir_list(
    ...
    dirs: Vec<crate::models::SetupDirectorio>,
    pool: &SqlitePool,
) {
